...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int x = allocate_semaphore_object(&sema);
	if( x != E_NO_SEMAPHORE)
	{
		strcpy(sema->name , semaphoreName) , sema->ownerID = ownerEnvID ;
	//	cprintf("the semaphore name %s \n",sema->name);
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	int semaphoreIDX =  get_semaphore_object_ID(ownerEnvID , semaphoreName);
	if ( semaphoreIDX != E_SEMAPHORE_NOT_EXISTS)
	{
		semaphores[semaphoreIDX].value -- ;
		if(semaphores[semaphoreIDX].value < 0)
		{
		    sched_remove_ready(myenv);
            enqueue(&semaphores[semaphoreIDX].env_queue , myenv);
		    myenv->env_status  = ENV_BLOCKED ;
		 //   cprintf("ownerID : %d is Now BLocked <==== \n" , myenv->env_id);
	    }
	}
	else
	cprintf("semName : %s  , PID : %d *\nCan't find semaphore in wait <--- \n",semaphoreName, ownerEnvID);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if ( semaphoreIDX != E_SEMAPHORE_NOT_EXISTS)
	{
		...
		{
			struct Env  *block ;
		    block = dequeue(&semaphores[semaphoreIDX].env_queue);
		    sched_insert_ready(block);
		    myenv->env_status  = ENV_READY ;
		//    cprintf("ownerID : %d is Now Ready <==== \n" , myenv->env_id);
		}
	}
	else
	cprintf("semName : %s  , PID : %d ***Can't find semaphore in signal <==== \n", semaphoreName, ownerEnvID);
}
