...
//============== Page Fault Handler Algo Fifo , LRU , Clock
void unmapVir_From_Envirment(struct Env * curenv, uint32 virtual_add_IDX )
{
	uint32 *page_table , *virtual_add ;
	virtual_add= (uint32*)env_page_ws_get_virtual_address(curenv, curenv->page_last_WS_index) ;
	struct Frame_Info *ptr = get_frame_info(curenv->env_page_directory, (uint32*)virtual_add, &page_table);
	uint32 PAGE_PERM=  pt_get_page_permissions(curenv ,(uint32) virtual_add) ;
  	 if(PAGE_PERM & PERM_MODIFIED  )
		pf_update_env_page(curenv, virtual_add, ptr);

  	 unmap_frame(curenv->env_page_directory, virtual_add);
}

uint32 page_fault_handler_RepalcmentFIFO(struct Env * curenv)
{
	  return curenv->page_last_WS_index ;
}

uint32 page_fault_handler_RepalcmentLRU(struct Env * curenv)
{
  uint32 MinTime = 0 ,curr_time , MinTime_IDX;
  for(uint32 i = 0 ; i< curenv->page_WS_max_size ;i++)
  {
	  curr_time = env_page_ws_get_time_stamp(curenv ,i) ;
	  env_page_ws_get_time_stamp(curenv,i);
	   if(MinTime > curr_time)
	   {
		   MinTime_IDX = i ;
		   MinTime = curr_time ;
	   }
...
uint32 page_fault_handler_RepalcmentClock(struct Env * curenv)
{
	uint32 NonUsed_IDX =  0 ,Virtual_address , *Va_PageTable  ;
	while(1)
	{
		Virtual_address = env_page_ws_get_virtual_address(curenv,NonUsed_IDX);
		get_page_table(curenv->env_page_directory,(uint32*)Virtual_address,&Va_PageTable);
		if( !(Va_PageTable[PTX(Virtual_address)] & PERM_USED ))
			break ;
		else
		Va_PageTable[PTX(Virtual_address)] &= (~PERM_USED) ;
		NonUsed_IDX = (NonUsed_IDX+1) % curenv->page_WS_max_size ;
	}
return NonUsed_IDX ;
}
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if(!env_page_ws_is_entry_empty(curenv ,curenv->page_last_WS_index))
	{
		...
		if(isPageReplacmentAlgorithmFIFO())
			Victim_IDX = page_fault_handler_RepalcmentFIFO(curenv);

		else if (isPageReplacmentAlgorithmLRU())
			Victim_IDX = page_fault_handler_RepalcmentLRU(curenv);

		else if (isPageReplacmentAlgorithmCLOCK())
			Victim_IDX = page_fault_handler_RepalcmentClock(curenv);

		unmapVir_From_Envirment(curenv , Victim_IDX);
    }

       struct Frame_Info * Frame  =NULL ;
	    allocate_frame(&Frame);
	    map_frame(curenv->env_page_directory ,Frame ,(void*)fault_va,(PERM_PRESENT|PERM_WRITEABLE|PERM_USER));
	   int Is_page_in_PF = pf_read_env_page(curenv, (void *)fault_va);
	   if(Is_page_in_PF ==  E_PAGE_NOT_EXIST_IN_PF  &&fault_va < USTACKTOP && fault_va >= USTACKBOTTOM )
	      pf_add_empty_env_page(curenv, fault_va, 0);
	   else if (Is_page_in_PF ==  E_PAGE_NOT_EXIST_IN_PF )
		   panic("This Page Not Stack Page . \n ");
