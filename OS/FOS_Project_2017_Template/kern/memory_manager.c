...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	int perm = PERM_PRESENT|PERM_WRITEABLE|PERM_USER;
    if(PAGE_TABLE == NULL) return NULL ;

     memset(PAGE_TABLE,0,PAGE_SIZE);
     ptr_page_directory[PDX(virtual_address)] = (kheap_physical_address((uint32)PAGE_TABLE) | perm );

     tlbflush();

    // cprintf("Successful in creat Page Table \n ") ;
    return PAGE_TABLE ;
}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 tmp =ROUNDDOWN(virtual_address , PAGE_SIZE * 1024);
	uint32 va = virtual_address;
	int indx ;
	int max =e->page_WS_max_size;
	for(int i = 0 ; i< size ;i++)
	{
		...
		for(int k = indx+1; k < max ;k++)
		{
			if(e->ptr_pageWorkingSet[max].empty == 1)
			{
				max--;
			}
			e->ptr_pageWorkingSet[k-1].empty = e->ptr_pageWorkingSet[k].empty;
			e->ptr_pageWorkingSet[k-1].virtual_address = e->ptr_pageWorkingSet[k].virtual_address;
			e->ptr_pageWorkingSet[k-1].time_stamp = e->ptr_pageWorkingSet[k].time_stamp;
			env_page_ws_clear_entry(e , k);
		}
		va += PAGE_SIZE;
	}
	 va = virtual_address;
	...
	{
		uint32 *ptr = NULL;
		get_page_table(e->env_page_directory ,(void *)va , &ptr);
		if(ptr == NULL)
		{
			va+= PAGE_SIZE;
			continue ;
		...
		if(cnt == 1024)
		{
			unmap_frame(e->env_page_directory , ptr);
