...
void * create_user_page_WS(unsigned int numOfElements)
{
  ...
  // cprintf("Successful creat Page_user \n");
	struct WorkingSetElement *User_Page = kmalloc(numOfElements*sizeof(struct WorkingSetElement));

    if(User_Page ==NULL) return NULL ;
...
void * create_user_directory()
{
	...
	//cprintf("Successful user_directory \n");
        uint32 *page_directory =  kmalloc(PAGE_SIZE);
